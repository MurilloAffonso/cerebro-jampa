...
const SLUGS_SEM_MARE = [
  { categoria: "litoral-sul",  slug: "roteiro-classico" },
  { categoria: "litoral-norte", slug: "por-do-sol-jacare" },
  { categoria: "city-tour",    slug: "jampa-historica" },
] as const;
...
export function TabuaMarePasseiosRelacionados() {
  ...
  const listaSemMare = SLUGS_SEM_MARE
    .map(({ slug }) => passeios.find((p) => p.slug === slug))
    .filter((p): p is Passeio => Boolean(p));
  ...
  return (
    ...
    >
      ...
      {/* Seção: passeios que não dependem da maré */}
      {listaSemMare.length > 0 && (
        <div className="container-safe mt-10">
          <header className="mb-6">
            ...
            >
              Maré alta? Estes passeios funcionam qualquer dia
            </h2>
            <p
              style={{
                fontFamily: "var(--font-inter)",
            ...
            >
              Não dependem da maré e saem com qualquer condição de mar.
            </p>
          </header>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            {listaSemMare.map((p) => {
              const href = urlPasseio(p);
              const waUrl = buildWaUrl(p.nome);
              return (
                <article
                  key={p.id}
